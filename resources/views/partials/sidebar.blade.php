...
<div v-if="sidebar" class="">
    ...
    <div class="fixed inset-0 flex z-40">
        <div  v-if="sidebar" class="flex-1 flex flex-col max-w-xs w-full bg-white transform ease-in-out duration-300 ">
                ...
                @foreach($menus as $menu)
                    @if($menu->hasSubMenu())
                        ...
                        @foreach($menu->subMenu($menu->key()) as $subMenu)
                            <a href="#" class="mt-3 group flex items-center px-12 py-2 text-sm text-base leading-6 font-light text-gray-900 rounded-md bg-gray-100 focus:outline-none focus:bg-gray-200">
                                {{ $subMenu->label() }}
                            </a>
                        @endforeach
                    @endif

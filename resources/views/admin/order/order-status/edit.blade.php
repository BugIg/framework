...
@section('pageTitle')
    {{ __('avored-admin::order.order-status.edit-title') }}
@endsection
...
@section('actions')
    <div class="flex justify-start">
         <span class="inline-flex rounded-md shadow-sm">
            <button type="submit"
                    onclick="document.getElementById('orderStatus-save').submit()"
...
@section('content')
    ...
    <div class="w-full mx-auto px-4 sm:px-6 md:px-8">
        <div class="py-4">
            <form id="orderStatus-save" method="post" action="{{ route('admin.order-status.update', $orderStatus->id) }}">
                @csrf
                @method('put')

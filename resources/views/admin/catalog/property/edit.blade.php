...
@section('pageTitle')
    {{ __('avored-admin::catalog.property.edit-title') }}
@endsection
...
@section('content')
    ...
    <div class="w-full mx-auto px-4 sm:px-6 md:px-8">
        <div class="py-4">
            <form id="property-save" method="post" action="{{ route('admin.property.update', $property->id) }}">
                @csrf
                @method('put')

                <avored-property-fields :prop-property="{{ $property }}" inline-template>
                    <div>
                        @foreach ($tabs as $tab)
